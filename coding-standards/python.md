# Python coding standards — mine-flow
...
- **Layout:** for a packaged/distributable library, prefer the `src/` layout (package under
  `src/mine-flow/`, tests in a top-level `tests/`) — it keeps tests running against the
...
- Raise specific exceptions; define a small package-level hierarchy
  (`class mine-flowError(Exception)`) and derive from it so callers can catch by domain.
