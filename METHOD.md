...
Everything lives in the docs hub: `Code/mine-flow-docs/`.
...
`Code/mine-flow-docs/templates/architecture-sessions/NN-*.md` and follows it exactly:
...
project and is never rewritten. `Code/mine-flow-docs/` describes *what the system is now*
...
`mine-flow-docs`) is its own **sibling** repo. Nothing sits "above" those repos except a
...
`Code/mine-flow-docs/templates/repo-readme-template.md` once the architecture names them. (Or choose
...
**All durable content lives in a repo** — almost always `Code/mine-flow-docs/`. The
workspace root holds only **per-machine** files: the pointer `CLAUDE.md` / `AGENTS.md`
(which redirect to the canonical `Code/mine-flow-docs/AGENTS.md`) and `.claude/` config.
These are not versioned (the root is not a repo) and are regenerated on each developer's
machine by `Code/mine-flow-docs/scripts/setup-workspace.sh`.
...
this file) write paths **relative to the workspace root** — `Code/mine-flow-docs/architecture/…`,
...
`prompts/` and `Code/mine-flow-docs/` as folders inside it rather than sibling repos. In
...
| STEP plan | `mine-flow-STEP-N-PLAN.md` | `acme-STEP-1-PLAN.md` |
| Substep prompt | `mine-flow-STEP-N.M-PROMPT.md` | `acme-STEP-1.5a-PROMPT.md` |
