...
|---------|--------------------|---------------|-------------|
| [`check-in.md`](check-in.md) | Doc-drift reconciliation (both directions), conditional-session coverage, accepted risk/debt review, and a full test run, with the completed report under `reports/`. | A **STEP**, roughly every 10–20 STEPs; *"run the check-in."* | `METHOD.md` §5 (*Check-in STEPs*); `AGENTS.md`; `registries/risks.yml` |
| [`impeccable-design-review.md`](impeccable-design-review.md) | Runtime evidence review of an implemented Flutter UI against the canonical UI/design-system contract, including responsive, interaction, accessibility, theme, and localization checks. | Named verification substep for UI-heavy work, or a dedicated **Design Review STEP** for cross-screen/release/check-in scope; *"run the Impeccable design review."* | `architecture/07-ui-design-system.md`; UI-related ADRs; `DESIGN.md` / `PRODUCT.md` bridge; Test Strategy |
