...
marker near the top of `Code/mine-flow-docs/overview.md` (the `<!-- PROJECT-STATUS: … -->`
line). It has one of two values:

If `Code/mine-flow-docs/overview.md` does not exist, this is still the uninitialized
...
- **`not-started` → Kickoff mode.** The project hasn't been bootstrapped yet. **Begin the
  kickoff now without waiting to be asked**: read `Code/mine-flow-docs/BOOTSTRAP-PROMPT.md`
  ...
  `Code/mine-flow-docs/overview.md` for review; seed it from the one-line description in
  *What is mine-flow* below). The user should not have to pre-write `overview.md` or paste a
...
## What is mine-flow
internal tools as dashboard app to monitor cut/fill volume, land clearing area, crew attendance, work timeline, daily logging, inventory tracking, and data bucket
<!-- Filled during kickoff from overview.md. Keep to a tight paragraph; details live in
     the architecture docs. -->

## How this project is built
This project follows the method in **`Code/mine-flow-docs/METHOD.md`** — read it. In short:
...
- Durable docs live in `Code/mine-flow-docs/`:
...
`Code/mine-flow-docs/templates/architecture-sessions/NN-*.md` and follow it exactly:
...
Each session reads what it needs from disk (`Code/mine-flow-docs/overview.md` + earlier
...
`Code/mine-flow-docs/templates/planning-session.md` and follow it: it turns the locked
...
- `Code/mine-flow-docs/` — the docs hub (this repo). All durable content lives here.
- `prompts/` — `prompts/STEP-index.md` roadmap + archived STEP plans/substep prompts.
- `Code/mine-flow-*` — code repos, created as the architecture names them.
...
`Code/mine-flow-docs/scripts/apply-project-license.sh <new-repo-path>`. The authoritative
selection is in `Code/mine-flow-docs/.throughstone/project-license`; the helper validates
...
`Code/mine-flow-docs/ONBOARDING.md` for the ordered setup and first-contribution STEP path.
...
durable content almost always belongs in `Code/mine-flow-docs/`.
...
- Significant decisions become **ADRs** (`Code/mine-flow-docs/templates/adr-template.md`); never
...
- **Keep the docs true.** When implementation changes an architecture decision, update the
  affected `Code/mine-flow-docs/architecture/NN-*.md` and bump its Version Log, or write an
  ...
  `Code/mine-flow-docs/METHOD.md` §6).
- **Keep accepted risks visible.** Known, accepted risks and deferred technical debt live in
  `Code/mine-flow-docs/registries/risks.yml`. Add or update a row when security controls,
  ...
  report under `Code/mine-flow-docs/reports/incidents/`, or check-in report under
  `Code/mine-flow-docs/reports/` that carries the details; create that source first if it
  doesn't exist.
- **Document code as you write it.** Every class, function, and method gets a docstring;
  comment the *why* of non-obvious logic (see
  `Code/mine-flow-docs/coding-standards/README.md`).
- **Suggest a check-in every ~10–20 STEPs.** When about that many STEPs have passed since the
  ...
  `Code/mine-flow-docs/runbooks/check-in.md` (doc-drift reconciliation both ways,
...
- **Flag milestone docs at a phase/release.** When a phase completes or you cut a release,
  ...
  `Code/mine-flow-docs/templates/release-notes-template.md`; end-user docs are otherwise outside this
...
- **Always say what's next.** End every session/STEP by updating the index, then tell the
  ...
  running `./doctor.sh status` (or `Code/mine-flow-docs/scripts/status.sh`) — it runs the **next-action resolver**
...
Full conventions are in `Code/mine-flow-docs/runbooks/collaboration.md` — read it before any
