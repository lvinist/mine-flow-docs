...
The manifest lives in the docs hub at `Code/mine-flow-docs/.throughstone/manifest.yml`
...
- `upstream_area` names the logical scaffold area in the Throughstone source. For now, this
  guide only defines `docs-hub`, which maps to upstream `Code/mine-flow-docs/`.
- `upstream_path` is relative to `upstream_area`. For example, `upstream_area: "docs-hub"` plus
  ...
  `Code/mine-flow-docs/scripts/status.sh`.
...
2. Read `Code/mine-flow-docs/.throughstone/manifest.yml` (or
