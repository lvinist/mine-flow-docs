...
- **DailyLog:** Structured daily progress reports submitted by foremen. (Linked to User) Each log carries one aggregate hazard assessment (hazard_state in 'not_assessed' | 'none' | 'present', severity low/medium/high/critical required iff present, optional notes and corrective action, migration 20260912000001): the foreman must answer the hazard question before submitting — 'not_assessed' is never a valid submitted state, and a present hazard requires a severity (user-approved policy, 2026-09-12). Server-side, a transition trigger enforces exactly draft→submitted→approved with supervisor-only approval pinning approved_by to the authenticated user, and approved rows are immutable.
