...
- **System/e2e testing**: Embedded in the single Flutter repository using `integration_test`. Runs against a dedicated Supabase "Staging/Test" project environment.
- **CI gates**: Commits/merges are blocked by: Linting/Formatting checks, Static Analysis (`flutter analyze`), Unit/Integration Tests (must pass 100%), and a successful app Build.
...
- **End-to-End (E2E) Tests:** Low volume. Covers full critical user journeys (e.g., logging a daily report) by driving the UI and interacting with a staging database.
...
Since the project is built in a monorepo-style single Flutter app, the E2E tests live directly inside the app repository using the `integration_test` package. To avoid polluting production data, these tests run against a distinctly provisioned Supabase Test environment.
